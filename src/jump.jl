...
function piecewiselinear2(m::JuMP.Model, x::VarOrAff, d, fd1, fd2; method=defaultmethod())
    initPWL!(m)
    ...
    if (n != length(fd1)) || (n != length(fd2))
        error("You provided a different number of breakpoints ($n) and two function values at the breakpoints ($(length(fd1)) and $(length(fd2)))")
    end
    if n == 0
        error("I don't know how to handle a piecewise linear function with no breakpoints")
    end

    z1 = JuMP.@variable(m, lower_bound=minimum(fd1), upper_bound=maximum(fd1), base_name="z_1_$counter")
    z2 = JuMP.@variable(m, lower_bound=minimum(fd2), upper_bound=maximum(fd2), base_name="z_2_$counter")

    if n == 1
        JuMP.@constraint(m, x ==  d[1])
        JuMP.@constraint(m, z1 == fd1[1])
        JuMP.@constraint(m, z2 == fd2[1])
        return z1, z2
    end
    ...
    if method == :Incremental
        ...
        JuMP.@constraint(m, z1 == fd1[1] + sum(δ[i]*(fd1[i+1]-fd1[i]) for i in 1:n-1))
        JuMP.@constraint(m, z2 == fd2[1] + sum(δ[i]*(fd2[i+1]-fd2[i]) for i in 1:n-1))
        for i in 1:n-1
        ...
        JuMP.@constraint(m, sum(λ[i]*fd1[i] for i in 1:n) == z1)
        JuMP.@constraint(m, sum(λ[i]*fd2[i] for i in 1:n) == z2)
...
function add_sos2(m::JuMP.Model, n::Int64; method=:ZigZag)
    λ = JuMP.@variable(m, [1:n], lower_bound=0, upper_bound=1, base_name="λ_$counter")
    JuMP.@constraint(m, sum(λ) == 1)
    if method in (:Logarithmic,:Log)
        sos2_logarithmic_formulation!(m, λ)
